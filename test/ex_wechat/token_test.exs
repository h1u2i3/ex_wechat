defmodule ExWechat.TokenTest do
  ...
  test "expect get the data from server" do
    expect_response(@endpoint, "/token", [grant_type: "client_credential", appid: appid, secret: secret], @data)
  ...
  test "get access_token should write to cache" do
    ...
    assert access_token == File.read!(access_token_cache) |> String.trim
  end

  test "when cache exists should read from cache" do
    prepare_for_access_token_cache("token")
    expect_response(@endpoint, "/token", [grant_type: "client_credential", appid: appid, secret: secret],
  ...
  defp del_access_token_cache do
    if File.exists?(access_token_cache), do: File.rm!(access_token_cache)
  end
